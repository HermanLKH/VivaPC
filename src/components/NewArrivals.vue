      ...
      <div class="row row-cols-1 row-cols-md-3 g-4">
        <div v-for="set in sets" :key="set.id" class="col">
          <div class="card h-100 shadow-sm product-card">
            <!-- entire card (minus footer) links to detail page -->
            <router-link
              :to="`/product/${set.id}`"
            ...
            <!-- this button now drives the add-to-cart + toast logic -->
...
async function handleAdd(item: Bundle) {
  ...
  try {
    // adapt as needed if your cart store expects a Product type
    await cart.addItem({
      id:          String(item.id),
      name:        item.name,
      description: '',
